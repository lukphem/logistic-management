            ...
            <tbody>
                @forelse ($countries as $country)
                    <tr class="border-b border-line last:border-0 odd:bg-surface-0 even:bg-surface-50/50 hover:bg-[var(--brand-primary)]/5 transition-colors">
                        ...
                        <td class="px-5 py-3 text-ink-500">{{ $country->continent ?? '—' }}</td>
                        <td class="px-5 py-3 text-ink-500">{{ $country->countryRegion?->name ?? '—' }}</td>
                        ...
                        <td class="px-5 py-3 text-right">
                            ...
                            <form method="POST" action="{{ route('countries.destroy', $country) }}" class="inline" data-confirm="Remove this country? Its states/cities will simply become unassigned, not deleted.">
                                @csrf @method('DELETE')
                ...
                @empty
                    <tr><td colspan="6" class="px-5 py-8 text-center text-sm text-ink-500">No countries configured yet.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
