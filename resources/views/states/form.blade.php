        ...
        <div>
            ...
            <select name="territory_id" class="w-full rounded-md border border-line px-3 py-2 text-sm outline-none focus:border-[var(--brand-primary)]">
                ...
                @foreach ($territories as $territory)
                    <option value="{{ $territory->id }}" @selected(old('territory_id', $state->territory_id) == $territory->id)>{{ $territory->name }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-ink-500">Used to auto-determine the domestic zone tier for routes involving this state (Billing → Zone Mapping).</p>
        </div>

        <label class="flex items-center gap-2 text-sm text-ink-900">
            <input type="checkbox" name="has_airport" value="1" @checked(old('has_airport', $state->has_airport)) class="rounded border-line">
            Has an airport
        </label>
