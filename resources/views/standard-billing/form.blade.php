<x-layouts.app :title="$tariff->exists ? 'Edit Tariff' : 'Add Tariff'">
    ...
    @if (! $tariff->exists && ($routeType ?? null))
        <p class="mb-4 text-sm text-ink-500">Adding a <strong>{{ ucfirst($routeType) }}</strong> tariff — only matching service types are selectable below.</p>
    @endif
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div>
                ...
                <input type="number" step="0.01" min="0" name="max_weight_limit" value="{{ old('max_weight_limit', $tariff->max_weight_limit ?? $tariff->min_weight) }}"
                       class="w-full rounded-md border border-line px-3 py-2 text-sm outline-none focus:border-[var(--brand-primary)] focus:ring-2 focus:ring-[var(--brand-primary)]/20">
                <p class="mt-1 text-xs text-ink-500">The zone's charge covers up to this weight — anything heavier is billed in increments below. Set independently of Min/Max weight above.</p>
            </div>
            <div>
