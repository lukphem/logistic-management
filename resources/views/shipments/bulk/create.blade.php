            ...
            @if ($canCollectCash || $paystackEnabled || $canUseWallet)
                <div id="payment-method-section" class="rounded-lg border border-dashed border-line p-4">
                    <p class="mb-3 text-xs font-medium uppercase tracking-wide text-ink-500">
                        ...
                        <span id="payment-method-note-credit" class="normal-case text-ink-400 hidden">— defaults to deferred (invoiced later), but this account can still pay this batch now instead</span>
                    </p>
    ...
    <script>
        ...
        function syncPaymentMethodVisibility() {
            ...
            if (isCredit) {
                ...
                // client can still choose to pay this batch now
                // instead — all three options stay available.
                if (deferredLabel) deferredLabel.classList.remove('hidden');
                if (deferredInput && !paymentMethodSection.querySelector('input[name="payment_method"]:checked')) deferredInput.checked = true;
                options.forEach(el => el.required = false);
                requiredNote?.classList.add('hidden');
                creditNote?.classList.remove('hidden');
            } else {
                // No credit facility to fall back on — a real choice
                // between the two actual payment methods is required.
