            ...
            <tbody>
                @forelse ($serviceTypes as $serviceType)
                    <tr class="border-b border-line last:border-0 odd:bg-surface-0 even:bg-surface-50/50 hover:bg-[var(--brand-primary)]/5 transition-colors">
                        ...
                        <td class="px-5 py-3 text-ink-500">{{ $serviceType->route_type === 'third_party' ? 'Cross-Trade' : ucfirst($serviceType->route_type) }}{{ $serviceType->trade_direction ? ' ('.ucfirst($serviceType->trade_direction).')' : '' }}</td>
                        <td class="px-5 py-3">
