<x-layouts.app :title="$zone->exists ? 'Edit Zone' : 'Add Zone'">
    ...
    <form method="POST" action="{{ $zone->exists ? route('zones.update', $zone) : route('zones.store') }}" class="max-w-xl space-y-4 rounded-xl border border-line bg-surface-0 shadow-sm p-5">
        ...
        @php
            $currentAppliesTo = $zone->exists
                ? ($zone->applies_domestic && $zone->applies_international ? 'both' : ($zone->applies_international ? 'international' : 'domestic'))
                : 'domestic';
            $appliesToOptions = ['domestic' => 'Domestic', 'international' => 'International', 'both' => 'Both'];
        @endphp
            ...
            <div class="flex gap-3">
                @foreach ($appliesToOptions as $key => $label)
                    <label class="flex flex-1 cursor-pointer items-center gap-2 rounded-lg border border-line p-3 text-sm text-ink-900 has-[:checked]:border-[var(--brand-primary)] has-[:checked]:bg-[var(--brand-primary)]/5">
                        <input type="radio" name="applies_to" value="{{ $key }}"
                               @checked(old('applies_to', $currentAppliesTo) === $key)
                               onchange="document.getElementById('tier-field').style.display = this.value !== 'international' ? '' : 'none';">
                        {{ $label }}
                    </label>
                @endforeach
            </div>
            <p class="mt-1 text-xs text-ink-500">
                A zone can apply to both — e.g. a broad zone reused across domestic and international mapping. The
                tier picker below only applies to the domestic side.
            </p>
        </div>

        <div id="tier-field" style="{{ old('applies_to', $currentAppliesTo) !== 'international' ? '' : 'display:none' }}">
