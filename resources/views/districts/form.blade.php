<x-layouts.app :title="$district->exists ? 'Edit District/Area' : 'Add District/Area'">
    ...
    <form method="POST" action="{{ $district->exists ? route('districts.update', $district) : route('districts.store') }}" class="max-w-xl space-y-4 rounded-xl border border-line bg-surface-0 shadow-sm p-5">
        ...
        <div>
            ...
            <select name="onforwarding_classification_id" class="w-full rounded-md border border-line px-3 py-2 text-sm outline-none focus:border-[var(--brand-primary)]">
                ...
                @foreach ($classifications as $classification)
                    <option value="{{ $classification->id }}" @selected(old('onforwarding_classification_id', $district->onforwarding_classification_id) == $classification->id)>
                        {{ $classification->name }} (+{{ number_format($classification->surcharge_amount, 2) }})
                    </option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-ink-500">
                Takes priority over the city's own classification when a shipment specifies this exact district.
            </p>
        </div>
        ...
        <div>
            ...
            <input type="text" name="postal_code" value="{{ old('postal_code', $district->postal_code) }}"
        ...
        <div>
            <label class="mb-1 block text-sm font-medium text-ink-900">Route <span class="text-xs font-normal text-ink-500">(optional)</span></label>
            <select name="route_id" class="w-full rounded-md border border-line px-3 py-2 text-sm outline-none focus:border-[var(--brand-primary)]">
                <option value="">No route</option>
                @foreach ($routes as $routeOption)
                    <option value="{{ $routeOption->id }}" @selected(old('route_id', $district->route_id) == $routeOption->id)>{{ $routeOption->name }}</option>
                @endforeach
            </select>
            <p class="mt-1 text-xs text-ink-500">Foundation for future shipment sorting and driver allocation — not used by anything yet.</p>
        </div>
